...
const GameMenu: React.FC<GameMenuProps> = ({ 
  ...
  // This screen should appear for both Host and Joiner if a wager is pending
      ...
      return (
          <div className="flex flex-col items-center justify-center min-h-[400px] w-full max-w-md p-8 bg-slate-800 rounded-2xl shadow-2xl border border-yellow-500/30 animate-in fade-in zoom-in duration-300">
             ...
             <div className="text-slate-400 text-center mb-8">
                 {isHost 
                    ? <span>Deposit <span className="text-yellow-400 font-bold">{pendingBetAmount} CHECK</span> to lock the prize pool and generate your room code.</span>
                    : <span>The host has wagered <span className="text-yellow-400 font-bold">{pendingBetAmount} CHECK</span>. Deposit to match and enter the match.</span>
                 }
             </div>
             ...
             <div className="w-full space-y-3">
                 <button
                    ...
                    className="w-full py-4 bg-emerald-600 hover:bg-emerald-500 text-white font-bold rounded-xl transition-all flex items-center justify-center gap-3 shadow-lg shadow-emerald-900/40"
                 >
                    {isConnecting || isLoggingIn ? <Loader2 className="w-5 h-5 animate-spin" /> : <Lock className="w-5 h-5" />}
                    {!userAddress ? "Login to Secure Funds" : `Approve & Deposit ${pendingBetAmount} CHECK`}
                 </button>
                 {connectionError && <p className="text-red-400 text-sm text-center font-medium bg-red-400/10 p-2 rounded-lg">{connectionError}</p>}
                 <div className="text-center text-xs text-slate-500 bg-slate-900/50 py-2 rounded-lg">
                    Funds are secured by smart contract and paid out to the winner.
                 </div>
             </div>
          </div>
      )
        ...
        <div className="flex flex-col items-center justify-center min-h-[400px] w-full max-w-md p-8 bg-slate-800 rounded-2xl shadow-2xl border border-slate-700 animate-in fade-in zoom-in duration-300">
           <div className="mb-6 text-center">
             <div className="inline-flex items-center gap-2 px-4 py-2 bg-emerald-500/10 border border-emerald-500/30 rounded-full mb-4">
                 ...
                 <span className="text-sm font-bold text-emerald-400">Funds Locked</span>
             </div>
           ...
           {hostId ? (
            <div className="w-full flex items-center gap-2 p-4 bg-slate-900 rounded-lg border border-slate-600 mb-6 group">
               <code className="flex-1 text-3xl font-mono text-center tracking-widest text-purple-300 group-hover:text-white transition-colors">
                 {hostId}
               </code>
               <button onClick={handleCopy} className="p-2 hover:bg-slate-800 rounded-md text-slate-400 hover:text-white transition-colors">
  ...
  if (myDepositComplete && !opponentDepositComplete && myColor === 'b') {
      return (
        <div className="flex flex-col items-center justify-center min-h-[400px] w-full max-w-md p-8 bg-slate-800 rounded-2xl shadow-2xl border border-slate-700 animate-in fade-in zoom-in duration-300">
            ...
            <h3 className="text-xl font-bold text-white">Match Confirmed</h3>
            <p className="text-slate-400">Finalizing connection with host...</p>
        </div>
      )
  }
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[500px] w-full max-w-md p-8 bg-slate-800 rounded-2xl shadow-2xl border border-slate-700 animate-in fade-in zoom-in duration-300 relative">
      ...
      {userAddress && checkBalance && (
        <div className="absolute top-4 left-4 flex items-center gap-2 px-3 py-1.5 bg-slate-900/80 backdrop-blur-sm rounded-full border border-yellow-500/30 shadow-lg shadow-yellow-500/10">
      ...
      <div className="mb-8 text-center mt-6">
        ...
        <p className="text-slate-400">PvP Spells & Strategy</p>
      </div>
        ...
        <div className="w-full space-y-4">
          <button onClick={onLocalPlay} className="w-full group relative flex items-center justify-between p-4 bg-slate-700 hover:bg-slate-600 rounded-xl transition-all border border-slate-600 hover:border-purple-500">
            <div className="flex items-center gap-4">
              ...
              <div className="text-left"><h3 className="font-bold text-white">Local Pass-and-Play</h3><p className="text-xs text-slate-400">No tokens required</p></div>
            </div>
          ...
          <div className="w-full p-4 bg-slate-700 rounded-xl border border-slate-600">
             ...
             <div className="flex gap-2 mb-4">
                 ...
                 <button onClick={() => setWagerInput('5')} className="px-3 py-2 bg-slate-600 hover:bg-slate-500 rounded-lg text-xs font-bold transition-colors">5</button>
                 <button onClick={() => setWagerInput('10')} className="px-3 py-2 bg-slate-600 hover:bg-slate-500 rounded-lg text-xs font-bold transition-colors">10</button>
             </div>
             <button onClick={handleHost} className="w-full py-3 bg-purple-600 hover:bg-purple-500 text-white font-bold rounded-lg transition-colors flex items-center justify-center gap-2 shadow-lg shadow-purple-900/20">Host Match</button>
          </div>
          ...
          <button onClick={() => setView('join')} className="w-full group relative flex items-center justify-between p-4 bg-slate-700 hover:bg-slate-600 rounded-xl transition-all border border-slate-600 hover:border-cyan-500">
            <div className="flex items-center gap-4">
              ...
              <div className="text-left"><h3 className="font-bold text-white">Join Match</h3><p className="text-xs text-slate-400">Enter host code</p></div>
            </div>
            <ChevronRight className="w-5 h-5 text-slate-500 group-hover:text-white" />
          </button>
        </div>
      ...
      {view === 'join' && (
        <div className="w-full text-center space-y-6">
          <div className="space-y-2"><h3 className="text-lg font-bold text-white">Join Game</h3><p className="text-sm text-slate-400">Enter host's code</p></div>
          <div className="space-y-4">
             <input type="text" maxLength={4} value={joinInput} onChange={(e) => setJoinInput(e.target.value.toUpperCase())} placeholder="XXXX" className="w-full p-4 text-center text-3xl font-mono tracking-[0.5em] bg-slate-900 border-2 border-slate-600 rounded-lg focus:border-purple-500 focus:outline-none text-white placeholder-slate-700 uppercase" />
             {connectionError && <p className="text-red-400 text-sm bg-red-400/10 p-2 rounded-lg">{connectionError}</p>}
             <button onClick={() => onJoinGame(joinInput)} disabled={joinInput.length < 4 || isConnecting} className="w-full py-4 bg-purple-600 hover:bg-purple-500 disabled:bg-slate-700 disabled:text-slate-500 text-white font-bold rounded-xl transition-all flex items-center justify-center gap-2">
                {isConnecting ? <Loader2 className="w-5 h-5 animate-spin" /> : <Play className="w-5 h-5" />}
                Connect to Host
             </button>
          </div>
          <button onClick={() => setView('main')} className="text-sm text-slate-500 hover:text-white transition-colors">Back to Menu</button>
        </div>
      )}
    </div>
  );
